...
const Content = props => {
  ...
  const activeFigureRef = React.createRef()
  const fig = React.createRef()
  ...
  const onMouseDown = (e, figure, index) => {
    ...
    const target = activeFigureRef.current
    console.log('activeFig: ', activeId)
          ...
          props.figures.map((figure, index) => {
            ...
            return (
              figure.type === 'triangle'
                ...
                // треугольники
                <Triangle key={index}
                  activeFigureRef={figure.id === activeId ? activeFigureRef : fig}
                  classes={classes}
                  index={index}
                  figure={figure}
